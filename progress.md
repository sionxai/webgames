...
## 2026-07-31 — 기다려멍 R3 모바일 세로 레이아웃

- `max-width: 520px` 전용 최종 cascade에서 상단바를 76px 2행으로 재배치하고, 5개 도움말/배속 버튼의 40px 터치 크기를 유지했다.
- 목표 패널은 44px 이하의 다음 일정·완료 횟수·5개 진행 도트로 접고, 하우스 제목과 모바일 여백을 축소했다.
- 가상 스틱은 `top: calc(100dvh - 172px)`로 월드 액션바 다음·하단 내비게이션 이전 조작 띠에 두고, 투명 wrapper에서 스틱만 pointer event를 받게 했다. 행동 버튼은 같은 하단 띠의 오른쪽에 분리했다.
- 짧은 가로 화면은 `orientation: landscape`와 `max-height: 520px` 전용 2열로 바꿔 캔버스를 왼쪽, 상단 정보·일정·행동을 오른쪽에 배치했다.
- TODO: 지정 수용 명령 실행 및 정상 브라우저 환경에서 운용자 레이아웃 실측.
...
## 2026-07-24 — 월드 상호작용형 훈련게임 v5 승인

- Original prompts:
  - “강아지가 뭔가 반응을 말풍선이나 행동으로 보이면 이동해서 관찰버튼으로 조사하고 (원인)이 나오면 도구나 행동 단축키를 이용해 해결”
  - “글로그가 없어도 게임 메인 화면만 보고도 플레이가 충분히 되고”
  - “컴퓨터에 앉아서 특정버튼을 누르고 있으면 조금씩 게이지가 차는걸로”
  - “사료줄때도 이동해서 상호작용버튼 누르고 사료 골라서 넣어주고. 물도 그렇게”
- 사용자 승인: “진행해줘”
- 기준: `origin/main` `7a08eddb211ceb8dea932f1b9b1b2d8f948b8ed0`, 격리 브랜치 `codex/waitdog-world-interactions-v5-20260724`.
- root worktree의 `package.json`, `package-lock.json` 및 다른 게임의 미커밋·untracked 파일은 사용자/다른 작업으로 간주해 변경하지 않는다.
- 목표: 버튼·패널 중심 흐름을 월드 신호 → 근접 관찰 → 추정 원인 → 단축 행동으로 교체하고, 업무·사료·물도 해당 오브젝트 근처에서만 수행한다.
- 위험등급 초안: R2. 이동, snapshot, 경제·재고, 조우 상태, Canvas, 입력·반응형이 함께 바뀌므로 독립 Checker 사전 계약과 최종 검수를 수행한다.
- 구현 계약과 DoD는 `implementation_plan.md`의 `기다려, 멍! — 월드 상호작용형 훈련게임 v5`에 기록했다.

## 2026-07-25 — 월드 상호작용형 훈련게임 v5 완료

- 구현 완료: 신호 말풍선/하이라이트, 근접 `E` 관찰, 추정 원인 공개, `1~3` 대응, `Space/Q` 후속 보상, 결과 닫기를 같은 월드 화면에서 처리하도록 통합했다.
- 업무·생활 루프 완료: 컴퓨터 앞 `E` 착석 후 `R` 홀드 진행, 사료/물 그릇 근접 상호작용, 월드 대상 우선순위, 모바일 스틱·홀드 버튼, 하단 액션 바를 연결했다.
- 마지막 체감 보정: 빈 밥/물그릇은 실제로 비어 보이도록 렌더를 수정했다. 보호자 약 122×145px, 강아지 약 110×89px의 실제 렌더 크기를 기준으로 방별 화면 공간 충돌 영역을 적용하고, 새 시작 위치·수평/대각 접근·문 이동에서 보호자와 강아지가 겹치거나 강아지가 순간이동하지 않도록 고정했다.
- 정적 검증: `npx tsc --noEmit`, `npm run build`, `npm run test:contract`, `node .waitdog-contract-dist/scripts/waitdog-contract.js`, `git diff --check` 모두 exit 0. 기다려멍 계약은 `557 assertions`, Forge 계약은 `141 assertions`, production build는 `1,536 modules`를 실제 실행했다.
