...
${parts.join(', ')}`;
      ...
      async function loadOrInitializeProfile(firebaseUser){
        ...
        const fallbackBase = firebaseUser.isAnonymous ? `게스트-${uid.slice(0, 4)}` : `user-${uid.slice(0, 6)}`;
      ...
      async function applyProfileState(){
        ...
        resetRareAnimationState({ immediate: true });
        const uid = currentFirebaseUser.uid;
        const fallbackBase = currentFirebaseUser.isAnonymous ? `게스트-${uid.slice(0, 4)}` : `user-${uid.slice(0, 6)}`;
